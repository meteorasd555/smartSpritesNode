...
var getInfo = function(imagePath, callback) {
	var argument  =  _getArg_identify(imagePath),
		...
		getFirstFrameReg = /\{.+?\}/g; // duel with gif
	 
	 if(!fs.existsSync(imagePath)) {
	 	callback(null);
	 	logger.warn("imagefile: ----- " + imagePath + " ----- not exists");
	 	return;
	 }  
	...
	identify.stdout.on('data', function (data) {
	  ...
	  if( util.lang.getType(imagePath) === "Array") {
	  	  ...
	  	  if(dataStr.match(getFirstFrameReg).length > 1) {
	  	  	 dataStr = dataStr.match(getFirstFrameReg)[0];
	  	  }
...
function drawImage (info, callback) {
	var emt = new EventBus(),
		tempfolder = "temp" + new Date().getTime() + temporaryId++,
	...
	emt.on("tempFolderReady", function() {
		...
		logger.info("creating image of size " + info.canvas.width + "x" + info.canvas.height);
		
	});

	// dest path  exists and dest image making success in temporary folder	
	// then copy them to dest path
	emt.on("destfolderReady", "drawingDone", function() {
