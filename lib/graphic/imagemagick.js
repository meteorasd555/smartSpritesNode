...
*/
function _getArg_identify(imageUrl) {
	var arguments = [IDENTIFY.ARGUMENTS.FORMAT], param = [], p, propMap = IDENTIFY.PROPMAP;
	for(p in propMap) {
	...
	arguments = arguments.concat(imageUrl);
	return arguments;
};
...
/*
	@description wrap a string width two string
	@param [String | Array] string need to be wrapped or an Array of string
	@param [Function] callback funciton, which will be invoked as long as proccess 	
*/
var getInfo = function(imageUrl, callback) {
	var argument  =  _getArg_identify(imageUrl),
	...
	identify.stdout.on('data', function (data) {
	  ...
	  if( util.lang.getType(imageUrl) === "Array") {
...
function drawImage (info, callback) {
	var emt = new EventEmitter(),
		tempfolderNm = "temp" + new Date().getTime(),
		cmd = _getCmd_composite(info);

	emt.on("createTempFileDone", function() {

		exec(cmd, {cwd: tempfolderNm}, function() {
			fs.rename(path.join(tempfolderNm,info.fsInfo.file) ,  path.join(info.fsInfo.destPath, info.fsInfo.file), function() {
				emt.emit("allDone");	
			});
		});
	...
	emt.on("allDone", function() {
		callback();
		fsExt.rmdirCascade(tempfolderNm, function(arg) {
	...
	// create a temp dir
	fs.mkdir(tempfolderNm, function(arg) {
		if(arg !== null) {
			// make dir error
			return;
		}
		emt.emit("createTempFileDone");
	});
...
function convertFormat(src, dest, option, callback) {
	var TPL_CONVERTFORMAT = "convert -define png:format=png8 {img} {imgDest}", cmd;
